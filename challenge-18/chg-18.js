...
function cleanCpf(cpf){
  let cpfFormat = cpf.replace(/\s|['A-z''--''.']/g, '').replace(/(\d\d\d)(\d\d\d)(\d\d\d)(\d\d)/g, '$1.$2.$3-$4');
  return cpfFormat;
}
...
var regexTag = /<\w+>/g
var stringTags = "<div><section><blockquote>Texto <img /></blockquote></section></div>";
console.log(stringTags.match(regexTag));
...
var regexTagAberturaFechamento = /<\w+><\/\w+>/g;
var stringHtmlTags = "<div><ul><li></li><li></li><li><span></span></li></ul></div>";
console.log(stringHtmlTags.match(regexTagAberturaFechamento));
...
var regexTextoHTML = /<(\w+)>([^<]+)<\/\w+>/g;
var stringHtmlTexto = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>";
stringHtmlTexto = stringHtmlTexto.replace(regexTextoHTML, '<$1>O texto dentro da tag "$1" é "$2</$1>"\n');
console.log(stringHtmlTexto);
})();
